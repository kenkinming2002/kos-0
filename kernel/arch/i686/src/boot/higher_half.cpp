...
extern "C" void higher_half_main()
{
  ...
  auto& pageTables    = utils::deref_cast<core::PageTable[BOOT_PAGE_TABLE_COUNT]>(kernelPageTable);
  ...
  for(size_t pageFrameIndex=reinterpret_cast<uintptr_t>(kernel_physical_end) / 4096u; pageFrameIndex<BOOT_PAGE_TABLE_COUNT * core::PAGE_TABLE_ENTRY_COUNT; ++pageFrameIndex)
  {
    auto pageTablesIndex = pageFrameIndex / core::PAGE_TABLE_ENTRY_COUNT;
    auto pageTableIndex  = pageFrameIndex % core::PAGE_TABLE_ENTRY_COUNT;
