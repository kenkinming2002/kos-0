...
#define MULTIBOOT2_TAG_PARSE_STUB0(name) \
  WEAK int multiboot2_tag_##name##_parse() { return 0; }

#define MULTIBOOT2_TAG_PARSE_STUB1(name) \
  WEAK int multiboot2_tag_##name##_parse(struct multiboot_tag_##name * name) { return 0; }

#define MULTIBOOT2_TAG_PARSE_STUB2(name, struct_name) \
  WEAK int multiboot2_tag_##name##_parse(struct multiboot_tag_##struct_name * struct_name) { return 0; }

#define MULTIBOOT2_TAG_PARSE_CALL_ERROR_RETURN(exp) \
  if((status = exp)) return status

#define MULTIBOOT2_TAG_PARSE_CALL1(name) \
  MULTIBOOT2_TAG_PARSE_CALL_ERROR_RETURN(multiboot2_tag_##name##_parse())

#define MULTIBOOT2_TAG_PARSE_CALL2(name, tag) \
  MULTIBOOT2_TAG_PARSE_CALL_ERROR_RETURN(multiboot2_tag_##name##_parse((struct multiboot_tag_##name *)tag))

#define MULTIBOOT2_TAG_PARSE_CALL3(name, struct_name, tag) \
  MULTIBOOT2_TAG_PARSE_CALL_ERROR_RETURN(multiboot2_tag_##name##_parse((struct multiboot_tag_##struct_name *)tag))
...
int multiboot2_boot_information_parse(void *boot_information)
{
  int status;
